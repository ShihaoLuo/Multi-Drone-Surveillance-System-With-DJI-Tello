...
class Tell_Controller:
    def __init__(self):
        ...
        self.video
        self.start_time = str(time.strftime("%a-%d-%b-%Y_%H-%M-%S-%Z", time.localtime(time.time())))
        self.tello_list = []
    ...
    def drone_handler(self,tello, _queue):
        while True:
            ...
            self.tello.send_command(_command)
    ...
    def scan(self,num_of_tello):
        ...
        for x in range(len(self.tello_list)):
            t1 = Thread(target=self.drone_handler, args=(self.tello_list[x], self.execution_pools[x]))
...
try:
    # command("scan 1")
    scan(scan_num)
    video = tello_video.Tello_Video(tello_list)
    #video.thread_start()
    command("battery_check 20")
    command("correct_ip")
    for i in range(len(sn_list)):
        command(str(i + 1) + "=" + sn_list[i])
    #command("delay 2")
    command("*>streamon")
    #command("*>takeoff")
    #command("1>streamoff")
    ...
    for ip in manager.tello_ip_list:
        manager.socket.sendto('streamoff'.encode('utf-8'), (ip, 9001))
        manager.socket.sendto('land'.encode('utf-8'), (ip, 9001))
    save_log(manager)
    manager.close()
    video.close()
